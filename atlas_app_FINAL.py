...
.block-container{{max-width:1500px;padding:1.4rem 2.7rem 1.2rem}}
...
[data-testid="stTextInput"] input{{height:56px;border-radius:10px;font-size:1rem}}
[data-testid="stTextInput"] input:focus{{border-color:var(--red)!important;box-shadow:0 0 0 1px var(--red)!important}}
.stButton>button,.stDownloadButton>button{{background:var(--red);color:#fff;border:1px solid var(--red);border-radius:7px;font-weight:800;min-height:44px}}
...
.atlas-logo-small{{width:168px;height:auto;display:block}}
...
/* search home */
.home-wrap{{min-height:610px;display:flex;flex-direction:column;align-items:center;justify-content:center;margin-top:-3.5rem}}
.home-logo{{width:255px;max-height:235px;object-fit:contain;margin-bottom:2.7rem}}
.home-search{{max-width:955px;margin:0 auto}}
/* cards */
.atlas-card{{border:1px solid #39352f;border-radius:14px;background:linear-gradient(145deg,#090909,#050505);padding:2.25rem 2.25rem 1.4rem;box-shadow:0 14px 38px rgba(0,0,0,.35)}}
.result-top{{display:flex;justify-content:space-between;gap:2rem;align-items:flex-start}}
.result-cob{{font-size:2.15rem;font-weight:800;letter-spacing:-.025em;line-height:1.1;margin:.2rem 0 .65rem}}
.result-group{{font-size:1.1rem;color:#8b8b8b}}
.kicker{{font-size:.72rem;text-transform:uppercase;color:#858585;font-weight:800;letter-spacing:.08em}}
.status-box{{border:1px solid currentColor;border-radius:8px;padding:.75rem 1rem;font-size:1.05rem;font-weight:850;min-width:210px;text-align:center}}
.status-clear{{color:var(--green)}} .status-likely{{color:var(--amber)}} .status-unclear{{color:var(--red)}}
.elig-title{{margin:2.7rem 0 1rem;color:#979797;font-weight:850;font-size:.85rem}}
.elig-grid{{display:grid;grid-template-columns:repeat(4,1fr);border-bottom:1px solid #39352f;padding-bottom:1.5rem}}
.elig{{text-align:center;border-right:1px solid #302d29}} .elig:last-child{{border-right:none}}
.elig-name{{font-weight:850;font-size:1.05rem}} .elig-icon{{font-size:1.65rem;margin:.55rem 0 .25rem}} .elig-value{{font-size:1.05rem;font-weight:850}}
.yes{{color:var(--green)}} .no{{color:#d12b31}}
.definition{{padding:1.35rem 0 .65rem;border-bottom:1px solid #39352f;color:#aaa;line-height:1.45}}
.details-row{{text-align:right;color:#777;padding:.9rem 0 0;font-size:.92rem}}
/* uploader */
.upload-shell{{max-width:1020px;margin:3rem auto 4.5rem;border:1px solid #39352f;border-radius:15px;background:#070707;padding:2rem}}
[data-testid="stFileUploader"]{{background:#050505;border:1px dashed #8e8a84;border-radius:13px;padding:2.5rem 1rem}}
[data-testid="stFileUploaderDropzone"]{{background:transparent;border:0;min-height:260px}}
[data-testid="stFileUploaderDropzoneInstructions"] span{{color:#f4f4f4!important;font-size:1.15rem;font-weight:750}}
...
.metric-card{{border:1px solid #39352f;border-radius:10px;background:#090909;text-align:center;padding:1rem .5rem;min-height:122px}}
.metric-n{{font-size:2rem;font-weight:850}} .metric-label{{font-size:.88rem;font-weight:800;margin:.4rem 0}} .metric-sub{{font-size:.77rem;color:#999}}
...
if st.session_state.atlas_mode == "Search" and not st.session_state.search_query:
    ...
    st.markdown('<style>.home-wrap{min-height:330px;margin-top:0;margin-bottom:-2.2rem}</style>',unsafe_allow_html=True)
    ...
    if st.session_state.atlas_mode == "Search":
        ...
        if "batch_results" in st.session_state:
            ...
            with p1: page_size=st.selectbox("Rows per page",[10,25,50],index=0,key="page_size")
            pages=max(1,math.ceil(len(view)/page_size))
            with p2: page=st.selectbox("Page",list(range(1,pages+1)),format_func=lambda x:f"Page {x} of {pages}",key="page")
            start=(page-1)*page_size; page_df=view.iloc[start:start+page_size].copy()
